...
function Wo_AddNewAnnouncement($text) {
    ...
    if (Wo_IsAdmin($user_id) === false AND Wo_IsModerator($user_id) === false) {
        return false;
    }
    if (empty($text)) {
        return false;
...
function Wo_IsNameExist($username, $active = 0) {
    ...
    $named_files = array(
        ...
        "create-group",
        "group-setting",
        'communities', // added here
        'suggested-communities',
        'create-community',
        'community-setting',
        'request-community', // to here
        "create-page",
        "page-setting",
        "post",
        "new-game",
...
function Wo_GetSearchAdv($search_qeury, $type, $offset = 0, $limit = 0) {
    ...
    } elseif ($type == 'communities') { //added this
        if ($offset > 0) {
            $offset_to .= " AND `id` < {$offset} AND `id` <> {$offset} ";
        }
        $query = mysqli_query($sqlConnect, " SELECT `id` FROM " . T_COMMUNITIES . " WHERE ((`community_name` LIKE '%$search_qeury%') OR `community_title` LIKE '%$search_qeury%') AND `active` = '1' {$offset_to} ORDER BY `id` DESC LIMIT 10");
        if (mysqli_num_rows($query)) {
            while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_SendSMSMessage($to, $message) {
    ...
    elseif ($wo["config"]["sms_provider"] == "zigatext" && !empty($wo["config"]["zigatext_api_key"])) {
        include_once "assets/libraries/zigatext/vendor/autoload.php";
        $zigatext = new \Kemonai\Zigatext($wo["config"]["zigatext_api_key"]);
        try
        {
            $response = $zigatext->message->send(['sender' => $wo["config"]["sms_sender_name"],
                                            'message' => $message,
                                            'recipients' => Wo_Secure($to),
...
function Wo_GetSubCategories() {
    ...
    $wo['community_sub_categories'] = array(); //added this
    ...
    if (mysqli_num_rows($categories)) {
        while ($fetched_data = mysqli_fetch_assoc($categories)) {
            ...
            if ($fetched_data['type'] == 'community') { //added this
                $fetched_data['lang'] = $wo['lang'][$fetched_data['lang_key']];
                $wo['community_sub_categories'][$fetched_data['category_id']][] = $fetched_data;
            }
...
function Wo_RegisterNewCustomField($registration_data) {
    ...
    if ($query) {
        ...
        $types_array = array(
            "page" => T_PAGES,
            "group" => T_GROUPS,
            "community" => T_COMMUNITIES, //added this
            "product" => T_PRODUCTS
        );
