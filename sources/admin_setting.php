...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'remove_multi_community') { //added this
        if (!empty($_POST['ids'])) {
            foreach ($_POST['ids'] as $key => $value) {
                if (!empty($value) && is_numeric($value) && $value > 0) {
                    Wo_DeleteCommunity(Wo_Secure($value));
                }
            }
            $data = ['status' => 200];
            header("Content-type: application/json");
            ...
            if ($_POST['lang_key'] != 'other' && $_POST['lang_key'] != 'all_') {
                ...
                if (!empty($category)) {
                    ...
                    if ($_GET['type'] == 'community') { //added this
                        $db->where('category',$category->id)->update(T_COMMUNITIES,array('category' => 1));
                    }
                    if ($_GET['type'] == 'blog') {
                        $db->where('category', $category->id)->update(T_BLOG, array(
                            'category' => 1
    ...
    if ($s == 'delete_sub_category' && !empty($_POST['lang_key'])) {
        ...
        if (!empty($_GET['type']) && in_array($_GET['type'], $types)) {
            ...
            if (!empty($category)) {
                ...
                if ($_GET['type'] == 'community') {//added this
                    $db->where('sub_category',$category->id)->update(T_COMMUNITIES,array('sub_category' => ''));
                }
                if ($_GET['type'] == 'product') {
                    $db->where('sub_category', $category->id)->update(T_PRODUCTS, array(
                        'sub_category' => ''
