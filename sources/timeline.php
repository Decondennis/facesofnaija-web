...
if (isset($_GET['u'])) {
    ...
    if (in_array(true, $check_user)) {
        ...
        } else if ($check_user['type'] == 'community') { //added this
            $id                  = $community_id = Wo_CommunityIdFromCommunityname($_GET['u']);
            $wo['community_profile'] = Wo_CommunityData($community_id);
            $type                = 'community';
            $about               = $wo['community_profile']['about'];
            $name                = $wo['community_profile']['name'];
        }
    } else {
...
if ($wo['config']['communities'] == 0 && $type == 'community') { //added this
    header("Location: " . $wo['config']['site_url']);
    exit();
}
if (!empty($_GET['ref'])) {
