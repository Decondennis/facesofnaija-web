...
if (empty($error_code)) {
    ...
    if (empty($recipient_data)) {
		...
		if (!empty($data['joined_communities'])) {
			$response_data['joined_communities'] = Wo_GetUsersCommunities($recipient_data['user_id'], 50);
			foreach ($response_data['joined_communities'] as $key => $value) {
                $response_data['joined_communities'][$key]['is_joined'] = Wo_IsCommunityJoined($value['community_id'], $wo['user']['id']);
            }
		}
