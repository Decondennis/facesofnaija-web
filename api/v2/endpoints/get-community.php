...
if (empty($error_code)) {
    ...
    if (empty($recipient_data)) {
		...
		//added this
		if (!empty($data['my_communities'])) {
			$my_communities_offset = (!empty($_POST['my_communities_offset']) && is_numeric($_POST['mycommunities_offset']) && $_POST['my_communities_offset'] > 0 ? Wo_Secure($_POST['my_communities_offset']) : 0);
	        $my_communities_limit = (!empty($_POST['my_communities_limit']) && is_numeric($_POST['my_communities_limit']) && $_POST['my_communities_limit'] > 0 && $_POST['my_communities_limit'] <= 50 ? Wo_Secure($_POST['my_communities_limit']) : 20);
			$communities = Wo_GetMyCommunitiesAPI($my_groups_limit,$my_groups_offset,'DESC');
		...
		//added this
		if (!empty($data['joined_communities'])) {
			if (!empty($_POST['user_id'])) {
				$user_id = Wo_Secure($_POST['user_id']);
				$user = Wo_UserData($user_id);
				if (!empty($user)) {
					$joined_communities_offset = (!empty($_POST['joined_communities_offset']) && is_numeric($_POST['joined_communities_offset']) && $_POST['joined_communities_offset'] > 0 ? Wo_Secure($_POST['joined_communities_offset']) : 0);
			        $joined_communities_limit = (!empty($_POST['joined_communities_limit']) && is_numeric($_POST['joined_communities_limit']) && $_POST['joined_communities_limit'] > 0 && $_POST['joined_communities_limit'] <= 50 ? Wo_Secure($_POST['joined_communities_limit']) : 20);

					$communities = Wo_GetUsersCommunitiesAPI($user_id, $joined_communities_limit,$joined_communities_offset);
					foreach ($communities as $key => $community) {
					    $communities[$key]['members'] = Wo_CountCommunityMembers($community['id']);
					}
					$response_data = array(
		...
		if (!empty($data['communities'])) {
			$communities_offset = (!empty($_POST['communities_offset']) && is_numeric($_POST['communities_offset']) && $_POST['communities_offset'] > 0 ? Wo_Secure($_POST['communities_offset']) : 0);
	        $communities_limit = (!empty($_POST['communities_limit']) && is_numeric($_POST['communities_limit']) && $_POST['communities_limit'] > 0 && $_POST['communities_limit'] <= 50 ? Wo_Secure($_POST['communities_limit']) : 20);

			$communities = Wo_GetUsersCommunitiesAPI($user_id,$communities_limit,$communities_offset);
	        foreach ($communities as $key => $community) {
	            $communities[$key]['members'] = Wo_CountCommunityMembers($community['id']);
	        }
