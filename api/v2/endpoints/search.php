...
$community_offset = (!empty($_POST['community_offset'])) ? (int) $_POST['community_offset'] : 0;
...
$search_query3 = Wo_GetSearchAdv($options['query'], 'communities', $group_offset);
if (count($search_query3) > 0) {
    foreach ($search_query3 as $wo['result']) {
        $wo['result']['is_joined'] = (Wo_IsCommunityJoined($wo['result']['id'], $wo['user']['user_id'])) ? 'yes' : 'no';
        $communities[] = $wo['result'];
    }
}
