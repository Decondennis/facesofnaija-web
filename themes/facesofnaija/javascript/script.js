...
function Wo_GetMorePosts() {
  ...
  } else if(typeof ($('#posts').attr('data-story-community')) == "string") {
    community_id = $('#posts').attr('data-story-community');
...
function Wo_RegisterAddCommunity(user_id, community_id) {
  Wo_progressIconLoader($('#add-' + user_id).find('button'));
  $.get(Wo_Ajax_Requests_File(), {
    f: 'register_community_add',
    user_id: user_id,
    community_id: community_id
  }, function (data) {
    if(data.status == 200) {
